...
export function createTaskStore(documentStore, conversationStore, teamStore = null) {
  ...
  const api = {
    ...
    createFromConversation(conversationId, { title, description = '', createdBy = 'local-user', assignedTo = null, assigneeId = null } = {}) { if (assigneeId !== null) { if (!teamStore) throw new Error('Team store is required'); teamStore.validateAssignment(assigneeId); } const conversation = conversationStore.get(conversationId), label = String(title || conversation.messages[0]?.body || '').trim(); if (!label) throw new Error('Task title is required'); const id = `task-${Date.now().toString(36)}-${++sequence}`, now = new Date().toISOString(); tasks[id] = { id, websiteId: conversation.websiteId, sourceConversationId: conversation.id, reviewSessionId: conversation.reviewSessionId || null, pageId: conversation.pageId, componentInstanceId: conversation.componentInstanceId || null, title: label, description: String(description), status: 'todo', createdAt: now, createdBy: actor(createdBy), assignedTo: actor(assignedTo), assigneeId, updatedAt: now, completedAt: null }; order.push(id); selectedId = id; conversationStore.linkTask(conversationId, id); notify(); return id; },
    update(id, patch) { const value = task(id); if (patch.assigneeId !== undefined) { if (!teamStore) throw new Error('Team store is required'); teamStore.validateAssignment(patch.assigneeId, value.assigneeId); } if (patch.title !== undefined) { const title = String(patch.title).trim(); if (!title) throw new Error('Task title is required'); value.title = title; } if (patch.description !== undefined) value.description = String(patch.description); if (patch.assignedTo !== undefined) value.assignedTo = actor(patch.assignedTo); if (patch.assigneeId !== undefined) value.assigneeId = patch.assigneeId; value.updatedAt = new Date().toISOString(); notify(); },
