...
export function createConversationStore(documentStore) {
  ...
  const api = {
    ...
    create({ pageId, componentInstanceId = null, body, author = 'local-user', reviewSessionId = activeReviewSessionId }) { const document = documentStore.getState(), page = document.pages[pageId]; if (!page) throw new Error(`Unknown page: ${pageId}`); if (componentInstanceId && !page.componentInstanceIds.includes(componentInstanceId)) throw new Error(`Component is not on page: ${componentInstanceId}`); const id = `conversation-${Date.now().toString(36)}-${++sequence}`; conversations[id] = { id, websiteId: document.website.id, pageId, componentInstanceId, reviewSessionId: reviewSessionId || null, anchor: componentInstanceId ? { kind: 'component' } : { kind: 'page' }, status: 'open', createdAt: new Date().toISOString(), createdBy: actor(author), messages: [message(body, author)] }; order.push(id); selectedId = id; notify(); return id; },
