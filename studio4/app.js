...
for (const href of ['/studio4/responsive.css','/studio4/history.css','/studio4/library.css','/studio4/comments.css']) document.head.append(Object.assign(document.createElement('link'), { rel: 'stylesheet', href }));
...
const commentsDock = document.createElement('aside'); commentsDock.className = 'comments-dock'; commentsDock.innerHTML = '<header><div><small>Review lens</small><strong>Conversations</strong></div><button data-close-comments>×</button></header><div class="comment-filters"><select data-page-filter><option value="all">All pages</option><option value="current">Current page</option></select><select data-status-filter><option value="open">Open</option><option value="resolved">Resolved</option><option value="all">All statuses</option></select></div><button data-new-page-comment>＋ Page conversation</button><ol data-conversation-list></ol><section data-conversation-detail></section>'; document.body.append(commentsDock);
